...
- **Rodada de UX (2026-09-06)** — sem rolagem horizontal e cadastro rápido de lançamento:
  - Tabelas Filament (lançamentos, contas, cartões, parcelamentos, recorrências, categorias) com 3–5 colunas em vez de 6–10: informação secundária vira segunda linha (`description`), status vira ícone com tooltip, ações de linha viram menu `···`; no celular cada linha é um cartão de 3 linhas (título, valor, meta) em vez de 8 blocos rotulados (~470 px → ~97 px por lançamento).
  - Tabelas custom em Blade (orçamentos, 13 relatórios, extrato, fatura, parcelas, ocorrências, pendentes) com colunas por prioridade via container queries (`@container` + `@sm:`/`@lg:`/`@2xl:`/`@3xl:`), meta em 12 px embaixo da descrição e ações em ícone; páginas de visualização (conta, cartão, parcelamento, recorrência) em coluna única.
  - Form de lançamento (página e modal "Lançamento rápido") reordenado: tipo → valor (grande, foco inicial, teclado decimal) → data → descrição → conta/cartão → categoria → favorecido → status; competência, etiquetas, divisão, observações e anexos em "Mais detalhes" recolhido; Parcelamento e Repetição recolhidos até o toggle ligar. Modal de 1852 px para 788 px de altura, rodapé com "Salvar lançamento" sempre visível; abas de tipo quebram linha no celular; botão da topbar some abaixo de `md` (só o FAB).
  - Campo Valor na edição não mostra mais "R$ R$"; navegação de mês em botões de ícone; uma ação primária por cabeçalho (as demais em cinza) e cabeçalho que quebra linha em vez de cortar; assets do Filament publicados pelo `filament:assets` entram no `.gitignore`.
...
- Auto-registro configurável (#5): `Features::registration()` só entra em `config/fortify.php` quando `FORTIFY_REGISTRATION` (padrão `true`) estiver ligada; desligada, `/register` devolve 404 e o link “Criar conta” some da tela de login. Produção pessoal roda com a flag em `false`.
